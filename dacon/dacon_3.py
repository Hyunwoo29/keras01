...
x1_pad = pad_sequences(x1, padding='pre', maxlen = 11)
# ic(x1_pad.shape) # (45654, 13)

pred_pad = pad_sequences(pred, padding='pre', maxlen=11)
...
x_train, x_test, y_train, y_test = train_test_split(x1_pad, y, train_size=0.7, shuffle=True, random_state=60)
...
x_train = x_train.reshape(31957, 11, 1)
x_test = x_test.reshape(13697, 11, 1)
# ic(x_train.shape, x_test.shape) #  ic| x_train.shape: (31957, 11), x_test.shape: (13697, 11)
...
# model = Model(inputs=input, outputs=output)
model = Sequential()
model.add(Embedding(input_dim=31957, output_dim=130, input_length=11)) # 예시1  #length는 왠만하면 max length 길이에 맞춰줘라
...
model.fit(x_train, y_train, epochs=100, batch_size=120, verbose=1, validation_split=0.2, callbacks=[es, cp])
