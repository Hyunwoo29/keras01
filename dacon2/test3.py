...
path = './_data/'
# 데이터를 불러오는 방법
def load_dataset(path, seed = 2000) : 
    data_path = os.path.join(path, 'train.csv')
    data = pd.read_csv(data_path)
    data_texts = data['사업명'] + data['과제명'] + data['내역사업명'].fillna('.').astype('str')  + data['요약문_연구내용'].fillna('.').astype('str')
...
def modeling(seed):
    ...
    for train_idx, valid_idx in skf.split(data_set[0], data_set[1]):
        ...
        callbacks = [
            ...
            tf.keras.callbacks.ReduceLROnPlateau(monitor = 'val_loss', factor=0.8, patience = 3,),
    ...
    sample['label'] = p
    print(sample.head())
    sample.to_csv(f'aaa_{seed}_{round(s,5)}.csv',index=False)
    pred = [predictions, s]
    
    return pred

path = './_data/'
data_set, train_set, valid_set = load_dataset(path, seed = 2000)
test = pd.read_csv('./_data/test.csv')
test_set = test['사업명'] + test['과제명'] + test['내역사업명'].fillna('.').astype('str') + test['요약문_연구내용'].fillna('.').astype('str')
Ngram_range = (1,2)
top_k = 2000 # 최대 8만 단어 사용
...
sample.to_csv('bert_10.csv', index= False)
sample.head()